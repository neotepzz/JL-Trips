...
if (isset($_GET['accepttripid'])) 
	...
	if ($lgstrip == 1 && $lhstrip == 0 && $ffdtrip == 0)
		{
		// "LGS";
		mysql_query ("UPDATE `trips` SET `lgsheada`=1 WHERE `tripid`='$tripid'");
		}
	else if ($lgstrip == 0 && $lhstrip == 1 && $ffdtrip == 0)
		{
		// "LHS";
		mysql_query ("UPDATE `trips` SET `lhsheada`=1 WHERE `tripid`='$tripid'");
		}
	else if ($lgstrip == 0 && $lhstrip == 0 && $ffdtrip == 1)
		{
		// "FFD";
		mysql_query ("UPDATE `trips` SET `ffdheada`=1 WHERE `tripid`='$tripid'");
	 	}
	else
		{
		// "ERROR"
...
//
		...
		$body = "This Trip - $tripid has been ACCEPTED by $username.";
		$subject = "This Trip - $tripid has been ACCEPTED by $username.";
		$to = $email;
		mail($to, $subject, $body, $headers);
		}
	else
		{
		$acceptmessage ='<h2>Could not accept this Trip, please contact NSC...</h2>';
		};
//
...
//
	...
	while ($row4 = mysql_fetch_array($q4))
		{
		$lgshead = $row4['lgsheada'];
		$lhshead = $row4['lhsheada'];
		$ffdhead = $row4['ffdheada'];		
		$lgs = $row4['LGS'];
		$lhs = $row4['LHS'];
		$ffd = $row4['FFD'];
		}
...
//********* This is the if elseif statement that will determine when the pretrip is converted to a trip. ********
				...
				if ($lgs == 1 && $ffd == 0 && $lhs == 0)
						...
						if ($lgshead == 1)
							{
								mysql_query("UPDATE `trips` SET `tripfinalised`=1, `tripaccepted`=0, `tripdenied`=0, `tripsubmitted`=0 WHERE `tripid`='$tripid'");
								mysql_query("INSERT INTO `log` (`user`, `logtime`, `logdesc`, `logtripid`) VALUES ('$username', NOW(), 'Trip accepted - First Acceptance', '$tripid')");
							}
						}
					//just FFD
...
<html>
<head>
	<meta http-equiv="refresh" content="3;trip_awaitingapprovalfinal.php">
...
<body>
	<div id="page-container">
		...
		<div id="header"><h1>Trip Accept</h1></div>
		<div id="sidebar-a">
			<div class="padding">
			<h2>Help</h2>
