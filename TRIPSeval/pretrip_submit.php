...
mysql_query ("UPDATE `trips` SET `pretripaccepted`='0', `pretripdenied`='0', `pretripsubmitted`='1' WHERE `tripid`='$subid'");
$username = $_SESSION['username'];
mysql_query("INSERT INTO `log` (`user`, `logtime`, `logdesc`, `logtripid`) VALUES ('$username', NOW(), 'Submitted Pre Trip', '$subid')");
$submitmessage = '<h2>Submitting Your Pre Trip... Please Wait</h2>';
//email setup syntax is mail(to,subject,message,headers,parameters)
		...
		while ($row3 = mysql_fetch_array($q3))
			...
			$body = "This Pre Trip - $subid has been SUBMITTED by $username.";
			$subject = "This Pre Trip - $subid has been SUBMITTED by $username.";
			$to = $evmemail;
		...
		$from_email = "tripsemail@endowedschools.org";
		$headers = "From: $from_name <$from_email>";
		$body = "This Pre Trip - $subid has been SUBMITTED.";
		$subject = "This Pre Trip - $subid has been SUBMITTED.";
		$to = $email;
		mail($to, $subject, $body, $headers);
		$toplinks = "";
...
<html>
<head>
	<meta http-equiv="refresh" content="2;URL=pretrip_view.php">
