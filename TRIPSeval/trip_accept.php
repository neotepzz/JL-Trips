...
if (isset($_GET['accepttripid'])) 
	...
	if ($lgstrip == 1)
		{
		// "LGS";
		mysql_query ("UPDATE `trips` SET `lgsdepa`=1 WHERE `tripid`='$tripid'");
		}
	elseif ($lhstrip == 1)
		{
		// "LHS";
		mysql_query ("UPDATE `trips` SET `lhsdepa`=1 WHERE `tripid`='$tripid'");
		}
	elseif ($ffdtrip == 1)
		{
		// "FFD";
		mysql_query ("UPDATE `trips` SET `ffddepa`=1 WHERE `tripid`='$tripid'");
	 	}
	else
		{
		// "ERROR"
...
//********* This is the if elseif statement that will determine when the pretrip is converted to a trip. ********
				...
				if ($lgs == 1 && $ffd == 0 && $lhs == 0)
						...
						if ($lgsdep == 1)
							{
								mysql_query("UPDATE `trips` SET `tripaccepted`=1, `tripdenied`=0, `tripsubmitted`=0 WHERE `tripid`='$tripid'");
...
<html>
<head>
	<meta http-equiv="refresh" content="3;trip_awaitingapproval.php">
