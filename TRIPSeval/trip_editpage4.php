...
<table>
	...
	<tr>
		...
		<form action="trip_fileupload_check.php?fltripid=<?php echo $gettripid ?>" method="post" enctype="multipart/form-data">
	...
	<tr>
		...
		<?php
		//this code will check the directory for the trip, if there is no directory, it will display a message saying no files uploaded.
			if (isset($_GET['edittripid'])) 
			{
				$tripid = ($_GET['edittripid']);
			}
			else
			{
				$tripid = 143;
			};
			//if the code below is an array, we will have a for each loop to access each piece of information seperatel and display it
			$target_path = "uploads\\$tripid\\";
			$directory = 'uploads/' .$tripid. '';
			if (is_dir($directory)) {
	  				...
	  				if (is_array($filenames))
	  				{
		  				foreach ($filenames as $filename)
		  				{
		  					...
		  					else
							{
								...
								// TEST CASE echo "File:<a href='trip_fileopen.php?flotripid=" . $tripid . "&flotripfilename=" . $filename . "&flotrip_target_path=" . $target_path . "'>$filename</a><br>";
								echo 'File: <a href="' .$fullpath. '">' .$filename. '</a><br>'; 
							};
						}
					}
	    		// do something
		...
		?>
		<form action="trip_update.php?edittripidpage4=<?php echo $gettripid; ?>" method="post">
		<td colspan="2">
			<br>
			<br>
			<br>
