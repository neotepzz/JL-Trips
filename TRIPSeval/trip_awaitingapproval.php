...
$submitquery = mysql_query("SELECT * FROM `trips` WHERE `tripsubmitted`='1' AND `$school`='1' AND `tripaccepted`='0' AND `tripdenied`='0'") or die (mysql_error());
...
if($submitcount > 0) {
		...
		if ($school2 == 'LGS' && $lgsdep == 1) 
			{$lgstick = 1;}
		else {$lgstick = 0;};
		
		if ($school2 == 'LHS' && $lhsdep == 1) 
			{$lhstick = 1;}
		else {$lhstick = 0;};
		
		if ($school2 == 'FFD' && $ffddep == 1) 
			{$ffdtick = 1;}
		else {$ffdtick = 0;};
	...
	// this displays the accurate school information below in a mini table inside the status column.
			...
			{	//Create a variable for View Link
				$viewlink = "<a href='trip_viewtrip.php?viewtripid=$tripid'>View</a>";
	...
	//Now create the variable that will display the data.} 
			...
			{	$viewlink = "<a href='trip_viewtrip.php?viewtripid=$tripid'>View</a>";
	//Create a variable for the Accept Link
				$acceptlink = "<a href='trip_accept.php?accepttripid=$tripid'>Accept</a>&nbsp;&nbsp;";
	//Create a variable for the Denied Link.
				$deniedlink = "<a href='trip_deniedreason.php?denytripid=$tripid'>Deny</a>";
	//Now create the variable that will display the data.}}
				$givetriplist = 1;
			}
			else
