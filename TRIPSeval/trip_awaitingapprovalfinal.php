...
$submitquery = mysql_query("SELECT * FROM `trips` WHERE `tripaccepted`=1 AND `$school`=1 AND `tripdenied`=0 AND `tripfinalised`=0") or die (mysql_error());
...
if($submitcount > 0) {
		while($row = mysql_fetch_array($submitquery)) { // assign the query to the row, fetching the data into an array
			...
			$lhs = $row['LHS'];
			$ffd = $row['FFD'];
			$lgshead = $row['lgsheada'];
			$lhshead = $row['lhsheada'];
			$ffdhead = $row['ffdheada'];
			$school2 = $school;
		...
		if ($school2 == 'LGS' && $lgshead == 1) 
			{$lgstick = 1;}
		else {$lgstick = 0;};
		
		if ($school2 == 'LHS' && $lhshead == 1) 
			{$lhstick = 1;}
		else {$lhstick = 0;};
		
		if ($school2 == 'FFD' && $ffdhead == 1) 
			{$ffdtick = 1;}
		else {$ffdtick = 0;};
//below statement makes sure that the trips displayed in the end are not finalised.
if ($tripfinalised != 1)

{
	...
	// this displays the accurate school information below in a mini table inside the status column.
			...
			{	//Create a variable for View Link
				$viewlink = "<a href='trip_viewtripfinal.php?viewtripid=$tripid'>View</a>";
	...
	//Now create the variable that will display the data.} 
			...
			{	$viewlink = "<a href='trip_viewtripfinal.php?viewtripid=$tripid'>View</a>";
	//Create a variable for the Accept Link
				$acceptlink = "<a href='trip_finalaccept.php?accepttripid=$tripid'>Accept</a>&nbsp;&nbsp;";
	//Create a variable for the Denied Link.
				$deniedlink = "<a href='trip_finaldeniedreason.php?denytripid=$tripid'>Deny</a>";
	//Now create the variable that will display the data.}}
				$givetriplist = 1;
			}
			else
	...
	//Now create the variable that will display the data.}}
			...
			$trip_list = '<h2 align="center">There Are No Trips Awaiting Approval</h2>';
	};
...
<body>
	<div id="page-container">
		...
		<div id="header"><h1>Trips Awaiting Approval</h1></div>
		<div id="sidebar-a">
			<div class="padding">
			<h2>Help</h2>
		...
		<div id="content">
			<div class="padding">
				<table><tr class="trborder"><th>Created By</th><th>Trip Description</th><th>Trip Status</th><th>Created Date</th><th>View Trip</th><th>Accept/Deny Trip</th></tr>
				<?php echo $trip_list ?>
				</table>	
			</div>
		</div>
